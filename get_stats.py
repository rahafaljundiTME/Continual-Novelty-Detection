#calls the stat evaluator to run the open-stat model which extract the stat, to rapidly extract the stat without the extensive computation of ND performance
import argparse
import torch
import os.path
from stat_evaluator import STAT_Evaluator
...
parser.add_argument('--novelty_method', choices=['ODIN', 'Max_Softmax','Mahalanobis','Gen_based','Open_Sim','Open_Sim_perturb','Open_Stat'], default='ODIN', help='which novelty detection to use')
parser.add_argument('--novelty_magnitude', default=0.0014, type=float,
                    help='ODIN perturbation magnitude')
parser.add_argument('--novelty_temperature', default=1000, type=float,
...
novelty_extra_str="tinyimagenet_exp"
for arg in vars(opt):
    if  'novelty' in arg:
        #(not "novelty_temperatur" in arg) and
        if (not "novelty_max_beta" in arg) and  (not "novelty_warmup"  in arg) and (not "novelty_layer_index" in arg):
...
if opt.novelty_tuned_on_in_task:
    ...
    cnv_evaluate=STAT_Evaluator(cn_models,data_sequence,opt)#



cnv_evaluate.output_name = output_name
all_task_results=cnv_evaluate.evaluate(opt,res)
