...
"""
Evaluate ND performance based on different metrics and for the pairs of the 3 sets
"""
class CNV_Evaluator(object):
    ...
    def evaluate(self,opt,res,nvmodel=None):
        ...
        for task_index in range(start_index,len(self.data_sequence)):
            ...
            for inset_task_index in range(len(cn_model.In)):
                ...
                if inset_task_index<len(cn_model.Forgotten):
                    ...
                    in_forg_auc=Metrics.auroc(in_data_prob, forgotten_data_prob, len(dsets['test'].classes), nvmodel.end)

                    results.in_forg_auc.append(in_forg_auc)
                    ...
                    if len(cn_model.Out)>0:
                        ...
                        out_forg_auc=Metrics.auroc( forgotten_data_prob, all_out_data_probs[inset_task_index],len(dsets['test'].classes), nvmodel.end)

                        results.out_forg_auc.append(out_forg_auc)
            ...
            if hasattr(nvmodel.model.module, "embedding"):
                num_classes = nvmodel.model.module.classifier._modules[nvmodel.model.last_layer_index].fc.out_features
            else:
